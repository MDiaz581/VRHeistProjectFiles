...
public class SteeringWheel : CustomInteractible {
	...
	public SteamVR_Action_Single triggerAxis = SteamVR_Input.GetAction<SteamVR_Action_Single>("Trigger"); //Trigger button
	public SteamVR_Action_Boolean brakeButton = SteamVR_Input.GetAction<SteamVR_Action_Boolean>("Brake"); //Brake button 

	public FMODUnity.StudioEventEmitter brakeEmitter;
	...
	public void GrabUpdate(CustomHand hand){
		...
		//Debug.Log("Grabbing Wheel");

		if (hand.handType == SteamVR_Input_Sources.LeftHand)
		{
			triggerValue = triggerAxis.GetAxis(SteamVR_Input_Sources.LeftHand);

			brake = brakeButton.GetState(SteamVR_Input_Sources.LeftHand);


			if (brakeButton.GetStateDown(SteamVR_Input_Sources.LeftHand))
            {
				brakeEmitter.Play();
			}
			if (brakeButton.GetStateUp(SteamVR_Input_Sources.LeftHand))
			{
				brakeEmitter.Stop();
			}
		...
		if (hand.handType == SteamVR_Input_Sources.RightHand)
        {
			triggerValue = triggerAxis.GetAxis(SteamVR_Input_Sources.RightHand);

			brake = brakeButton.GetState(SteamVR_Input_Sources.RightHand);

			if (brakeButton.GetStateDown(SteamVR_Input_Sources.RightHand))
			{
				brakeEmitter.Play();
			}
			if (brakeButton.GetStateUp(SteamVR_Input_Sources.RightHand))
			{
				brakeEmitter.Stop();
			}
		}
        ...
        if (brake)
        {
			Debug.Log("Braking " + brake);
        } else
        {
			Debug.Log("Not Braking " + brake);
		}
